...
// ---- Debug mode: set to true to log URL routing and cleaning decisions ----
const DEBUG = false;
...
// JD cleaning: clear startup fields + scrub deep floorList for embedded ad data (promotionInfo, jdAdInfo etc).
function cleanJd(obj) {
  ...
  for (const root of [obj, obj?.data, obj?.result, obj?.result?.data]) {
    ...
    // Recursively scrub floorList contents — JD often nests ads inside floorList[].data
    if (Array.isArray(root.floorList)) {
      root.floorList = scrub(root.floorList);
    }
    root.countdown = 0;
    root.showTimesDaily = 0;
  }
